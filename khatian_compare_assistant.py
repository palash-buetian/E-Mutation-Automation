...
def credentials():
    ...
    username = config.get('ASSISTANT_LOGIN', 'username')
    password = config.get('ASSISTANT_LOGIN', 'password')
...
def prepare_browser():
    ...
    chrome_options.add_experimental_option("excludeSwitches", ['enable-automation', 'disable-logging'])
...
def new_case_found(processed=False):
    ...
    try:
        ...
        click_by_linktext("তুলনার জন্য অপেক্ষমাণ")
        return True
...
def send():
    ...
    # select the 1st order select option, option id =3
    select = Select(browser.find_element_by_id('desk_status'))
    select.select_by_value('3')
...
if is_connected("one.one.one.one"):
    ...
    if login(browser, credentials) is True:
        ...
        click_by_linktext("খারিজ খতিয়ান")
